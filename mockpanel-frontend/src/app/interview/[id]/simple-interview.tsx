...
export default function SimpleInterviewPage() {
  ...
  const wsUrl = useMemo(() => {
    ...
    return `${proto}//${window.location.host}/ws/v1/interview/${sessionId}`;
  }, [sessionId]);
  ...
  // ✅ FIX: const text = msg.text — TypeScript ab narrowing properly karta hai
  useEffect(() => {
    ...
    newMsgs.forEach((msg) => {
      ...
      if (msg.type === "question" || msg.type === "response_complete") {
        const text = msg.text; // narrow: string | undefined → string (after check)
        if (text) {
          setCurrentQuestion(text);
          setIsInterviewerThinking(false);
          setConversation((prev) => [...prev, { type: "question", text, timestamp: new Date() }]);
        }
  ...
  const submitAnswer = async () => {
    ...
    sendJson({ type: "text", text: userAnswer.trim() });
    setUserAnswer("");
    setIsInterviewerThinking(true);
        ...
        <div className="flex-1 overflow-y-auto pr-2 mb-6 space-y-5 min-h-[30vh]">
          {conversation.map((item, index) => (
            <div key={index} className="flex items-start space-x-3">
              {item.type === "question" ? (
                <>
                  <div className="w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-semibold flex-shrink-0 mt-1"
                    style={{ backgroundColor: interviewer.accentColor }}>
                    {interviewer.avatar}
                  </div>
                  <div className="flex-1">
                ...
                <>
                  <div className="flex-1 flex justify-end">
                    <div className="bg-blue-50 border border-blue-200 rounded-xl p-3.5 inline-block max-w-[85%]">
                      <p className="text-sm text-gray-900 leading-relaxed">{item.text}</p>
                    </div>
                  </div>
          ...
          {isInterviewerThinking && streamingText && (
            <div className="flex items-start space-x-3 animate-in fade-in slide-in-from-bottom-2">
              <div className="w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-semibold flex-shrink-0 mt-1 shadow-sm"
                style={{ backgroundColor: interviewer.accentColor }}>
                {interviewer.avatar}
              </div>
              <div className="flex-1">
                <div className="bg-gray-100 rounded-xl p-3.5 inline-block max-w-[85%] border border-gray-200 shadow-sm">
                  <p className="text-sm text-gray-900 leading-relaxed">
                    {streamingText}
                    <span className="inline-block w-1.5 h-3 ml-1 bg-gray-500 animate-pulse align-middle" />
                  </p>
                </div>
              </div>
            </div>
